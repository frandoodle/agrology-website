...
function aquiferPhosphorusStyle(feature) {
    return {
        fillColor: "#FF0000",
        fillOpacity: 0.2,
        color: '#FF4444',
...
function aquiferVulnerableStyle(feature) {
    return {
        fillColor: "#FFBF00",
        fillOpacity: 0.2,
        color: '#FFBF44',
...
var aquiferPhosphorusLayer = new L.geoJSON(aquiferPhosphorusData,{
    style: aquiferPhosphorusStyle,
    interactive: false,

});

var aquiferVulnerableLayer = new L.geoJSON(aquiferVulnerableData,{
    style: aquiferVulnerableStyle,
    interactive: false,

});

var municipalityLayer = new L.geoJSON(municipalityData,{
    style: municipalityStyle,
    interactive: false,
});

var alrLayer = new L.geoJSON(alrData,{
    style: alrStyle,
    interactive: false,
});

var fwaStreamsLayer = new L.geoJSON(fwaStreamsData,{
    style: fwaStyle,
    interactive: false,
});
...
var options = {
  ...
  var vtLayer = L.geoJson.vt(fwaStreamsData, options);
...
var layers = L.layerGroup([aquiferVulnerableLayer, aquiferPhosphorusLayer, municipalityLayer, alrLayer, vtLayer]);
layers.addTo(map);
...
map.on("click", function (e) {
    ...
    map.eachLayer(function(layer) {
        ...
        var inside =  turf.booleanPointInPolygon(point,layer.toGeoJSON());
        if (inside) {
            if(layer.feature.properties.AA_NAME){
                content += layer.feature.properties.AA_NAME;
            }
            if(layer.feature.properties.PHOSPHORUS) {
                content += "PHOSPHORUS"
            }
            if(layer.feature.properties.VULNERABLE) {
                content += "VULNERABLE"
            }
            if(layer.feature.properties.ALR) {
                content += "ALR"
            }
        }
    }
